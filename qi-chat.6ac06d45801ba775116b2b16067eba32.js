...
</svg></a>`:""}</div>`}):""}${i===-1&&a?f`<div class="user"><p>${a}</p></div>`:""}${i===-1&&s&&s.length>0?f`<div class="assistant">${s[0]===N?f`<div id="dots">${s}</div>`:f`<p>${l(s,"assistant",g)}</p>`}</div>`:""}${this._pane({placeholder:c>0?y`${this.lang}${t-h.length}Session`:"",size:c,disabled:s&&s.length>0,ime:o},g)}`}};C(V,"properties",{cells:{type:Array},api:{type:Object},lang:{type:String},for:{type:String},_tid:{type:Object},key:{type:Number},_current:{type:Number},_asking:{type:String},_waiting:{type:Array},_stage:{type:HTMLElement},_ime:{type:String},_plugins:{type:Array}}),C(V,"styles",nt`
  :host {
    display: block;
    font-size: 1rem;
    padding-top: 2rem;
